...
int f_pc = [
	...
	M_icode == IJXX && !M_Cnd : M_valA;
        M_icode == IRMMOVL && D_icode == IJXX && D_ifun == 1  && !M_Cnd && !(E_icode in {IOPL,IIADDL}) : D_valP;
...
bool F_stall =
	...
	# Stalling at fetch while ret passes through pipeline
        (IRET == D_icode && IPUSHL != E_icode) || IRET in {E_icode,M_icode} ;
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVL, IPOPL } && E_dstM in { d_srcA, d_srcB }) &&
        (IRET in { E_icode, M_icode } || (IRET == D_icode && IPUSHL != E_icode)) ;
...
bool E_bubble =
	...
	# Conditions for a load/use hazard
	E_icode in { IMRMOVL, IPOPL } &&
	 E_dstM in { d_srcA, d_srcB}) || (D_icode == IRET && E_icode == IPUSHL) ||(M_icode == IRMMOVL && D_icode == IJXX && D_ifun == 1 && !(E_icode in {IOPL,IIADDL}));

# Should I stall or inject a bubble into Pipeline Register M?
# At most one of these can be true.
bool M_stall = 0;
# Start injecting bubbles as soon as exception passes through memory stage
bool M_bubble = m_stat in { SADR, SINS, SHLT } || W_stat in { SADR, SINS, SHLT } || (D_icode == IRET && E_icode == IPUSHL);
