...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */
        if(cond_doit(sim->cc,ifun2)){
           argA = get_reg_val(sim->r,regA);
           set_reg_val(sim->r,regB,argA);
        }
        sim->pc = next_pc;
        break;
      ...
      case I_RMMOVL: /* 4:0 regA:regB imm */
        argA = get_reg_val(sim->r,regA);
        argB = get_reg_val(sim->r,regB);
        set_long_val(sim->m,argB+imm,argA);
        sim->pc = next_pc;
        break;
      case I_MRMOVL: /* 5:0 regB:regA imm */
        argB = get_reg_val(sim->r,regB);
        if(!get_long_val(sim->m,argB+imm,&val)){
           err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, argB+imm);
           return STAT_ADR;
        }
      ...
      case I_JMP: /* 7:x imm */
        if(cond_doit(sim->cc,ifun2)){
           if (imm < 0 || imm >= sim->m->len){
              err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, imm);
              return STAT_ADR;
           }
           sim->pc = imm;
        }
      ...
      case I_CALL: /* 8:x imm */
        ...
        val = get_reg_val(sim->r,REG_ESP);
        set_reg_val(sim->r,REG_ESP, val-4);
        if(!set_long_val(sim->m,val-4,next_pc)){
           err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, val-4);
              return STAT_ADR;
        }
        sim->pc = imm;
        break;     
      ...
      case I_PUSHL: /* A:0 regA:F */
        ...
        val = get_reg_val(sim->r,REG_ESP);
        set_reg_val(sim->r,REG_ESP, val-4);
        if(!set_long_val(sim->m,val-4,argA)){
           err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, val-4);
           return STAT_ADR;
        }
        sim->pc = next_pc;
